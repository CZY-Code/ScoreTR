...
class _ScoreTRBase(nn.Module):
    ...
    def setup_energy_(self):
        ...
        self.U_Net_sigma = MLP(input_dim=self.posdim, output_dim=self.rank,
                         h_dim=self.h_dim, act=nn.ReLU(), dropout=self.dropout,
                         bn=False, wn=False, sn=False, 
                         skip_connection=self.skip_connection)
        ...
        self.encoding = rff.layers.GaussianEncoding(alpha=1.0, sigma=8.0, input_size=1, encoded_size=self.posdim//2)
    ...
    def forward(self, idx, x, sample=True, return_z=False):
        ...
        mu = []
        sigma = []
        # BUG 如何避免输入同的坐标，但是输出的latent相同的情况？
        # 需要Z的分布满足标准高斯吗？从而对约束U_Net的输出
        # 下面网络的激活函数很重要，还需要调整
...
class ScoreTRContinuous(_ScoreTRBase):
    ...
    def loss(self, idx, x):
        ...
        # first term
        phi_xz, z_x = self.forward(idx, x, sample=True, return_z=True)
        # log_q_z_x = 0.
        # for d in range(self.dim):
            # q_z_mu = self.q_z_mu[d][idx[:, d]]
            # q_z_sigma = self.q_z_sigma(d)[idx[:, d]]
            # log_q_z_x = log_q_z_x + gaussian_log_prob(x=z_x[d], x_mu=q_z_mu, x_sigma=q_z_sigma).sum(-1)
        ...
        for _ in range(self.nu):
            ...
            phi_yz, z_y = self.forward(idx, y, sample=True, return_z=True)
            # log_q_z_y = 0.
            # for d in range(self.dim):
                # q_z_mu = self.q_z_mu[d][idx[:, d]]
                # q_z_sigma = self.q_z_sigma(d)[idx[:, d]]
                # log_q_z_y = log_q_z_y + gaussian_log_prob(x=z_y[d], x_mu=q_z_mu, x_sigma=q_z_sigma).sum(-1)
    ...
    def langevin_sample_posterior(self, idx, x0=None, step=1000, epsilon=0.02, t=1.0):
        def energy(x):
    ...
    def grid_search_posterior(self, idx, x_range, epsilon):
        ...
        x_grid = torch.arange(start=x_range[0], end=x_range[1] + epsilon / 2., step=epsilon, 
                              device=dev, dtype=torch.float).view(-1, 1).repeat(1, len(idx))  # Grid x Batch

        def unnorm_log_prob(x):
            out = self.forward(idx, x, sample=False, return_z=False)
            return out

        u_log_prob = torch.vmap(unnorm_log_prob, in_dims=0)(x_grid)

...
class EnergyTDIdx(nn.Module):
    def __init__(
        ...
        self.noise_sigma = sigma_begin
        # self.nosie_sigma_list = torch.from_numpy(np.linspace(noise_sigma, 0.01, 10)).float() #sigma从大到小
        self.nosie_sigma_list = torch.from_numpy(np.linspace(sigma_begin, sigma_end, sigma_level)).float() #sigma从大到小
    ...
    def grid_search_posterior_idx(self, idx, x_range, epsilon): #x_range=[-0.5, 1.1] epsilon=1e-3
        assert x_range is not None, "Please specify sample range!"
        dev = self.q_z_mu[0].device
        dtype = self.q_z_mu[0].dtype
        x_grid = torch.arange(start=x_range[0], end=x_range[1], step=epsilon,
                              device=dev, dtype=dtype).view(-1, 1).repeat(1, len(idx))  # Grid x Batch
        
        def unnorm_log_prob(x):
            neg_energy = self.forward(idx, x, sample=False, return_z=False)
            return neg_energy
        u_log_prob = torch.vmap(unnorm_log_prob, in_dims=0)(x_grid) #[1000, 1]
        return u_log_prob.squeeze(), x_grid.squeeze()

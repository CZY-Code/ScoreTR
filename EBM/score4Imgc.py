...
class ScoreEstimation(nn.Module):
    def __init__(
        self, tensor_shape, rank, h_dim, act, dropout,
        latent_dim, x_emb_size, sigma_func, 
        sigma_begin, sigma_end, sigma_level,
        pooling_method, skip_connection, posdim, dtype,
        rff_alpha, rff_sigma):
        super(ScoreEstimation, self).__init__()
        self.dtype = dtype
        ...
        if sigma_level is not None:
            ...
            self.label_embed = nn.Embedding(num_embeddings=sigma_level, embedding_dim=1)
        ...
        if self.sigmas is not None:
            label_enc = MLP(input_dim=emb_size, output_dim=self.latent_dim,
                            h_dim=self.h_dim, act=act, dropout=self.dropout,
                            bn=False, wn=False, sn=False, 
                            skip_connection=self.skip_connection)
            in_size += self.latent_dim
    ...
    def forward(self, idx, x, labels=None, sample=True):
        z = []
        mu = []
        # sigma = []
    ...
    def anneal_score_matching(self, idx, samples, labels, anneal_power=2.0):
        ...
        scores = self.forward(idx, perturbed_samples, perturbed_labels) #[n, 1]
    ...
    def anneal_langevin_dynamics(self, idx, pts, step_lr, n_steps): #(1e-4, 10)
        ...
        for c, sigma in enumerate(self.sigmas):
            ...
            for i in range(n_steps):
                # pts = pts + step_size / 2 * self.forward(idx, pts, lables, sample=False).detach()
                # pts = pts + torch.randn_like(pts) * np.sqrt(step_size)
                pts = pts + step_size * self.forward(idx, pts, lables, sample=False).detach()
